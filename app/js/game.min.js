...
function end(){if(gameOverScene.visible=!0,null!=flagCollision){plane.rotation-=.03,plane.y+=Position.STEP_Y;for(var e=0;e<enemy.length;e++)e!=flagCollision?enemy[e].movieclip.y+=Position.STEP_Y/2:(enemy[e].movieclip.rotation+=.03,enemy[e].movieclip.y+=Position.STEP_Y)}gameOverScene.addChild(score),gameScene.removeChild(score),(score.x<=renderer.width/2-score.width/2||score.y>=renderer.height/3-message.height)&&(score.x+=2.7,score.y-=3),score.scale.x<=1.2&&score.scale.y<=1.2&&(score.scale.x+=.002,score.scale.y+=.002),gameScene.filters=[blurFilter],filtersValue+=.005,blurFilter.blur=10*Math.sin(filtersValue)}
